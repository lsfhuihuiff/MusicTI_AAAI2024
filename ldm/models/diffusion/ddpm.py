...
class LatentDiffusion(DDPM):
    ...
    def __init__(self,
        ...
        for i in self.embedding_manager.embedding_parameters():
            for param in i:
    ...
    def get_learned_conditioning(self, c, timestep=None):
        if self.cond_stage_forward is None:
            if hasattr(self.cond_stage_model, 'encode') and callable(self.cond_stage_model.encode):
                c = self.cond_stage_model.encode(c, embedding_manager=self.embedding_manager, timestep=timestep)
        ...
        if sample:
            ...
            with self.ema_scope("Plotting"):
                samples, z_denoise_row = self.sample_log(cond=c,batch_size=N,ddim=use_ddim,
                                                         ...
                                                         get_learned_conditioning=self.get_learned_conditioning)
    ...
    def configure_optimizers(self):
        ...
        if self.embedding_manager is not None: # If using textual inversion
            embedding_params = self.embedding_manager.list_embedding_parameters()
    ...
    def configure_opt_embedding(self):
        ...
        lr = self.learning_rate
        params = self.embedding_manager.list_embedding_parameters()
